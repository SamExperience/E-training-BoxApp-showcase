...
*The application's entry point featuring a clean, minimalist interface. Authentication is secured via Google OAuth 2.0, ensuring a smooth and rapid user experience right from the first contact.*

![Authentication & Welcome](./assets/01-authentication-welcome.png)

### 2. Profile Configuration
*User profile configuration interface (weight, fitness level, boxing stance). This data is essential for allowing the AI to dynamically adapt exercise intensity and personalize the vocal coaching.*

![Profile Configuration](./assets/02-profile-configuration.png)

### 3. Home Page & History
*Centralization of athlete statistics. The fixed profile card displays key metrics, while the history section allows users to review the duration, number of rounds, perceived intensity, and satisfaction level for each completed session.*

![Home Page & History](./assets/03-homepage-history.png)

### 4. Program Selection
*Dynamic selection menu offering different themes (Cardio, Fitness, Free Training). Each mode features its own color code and a custom-generated workout structure tailored to meet the user's specific goals.*

![Program Selection](./assets/04-program-selection.png)

### 5. Workout Interface
*The core of the BoxApp. An interface optimized for vertical screens displaying the timer, punch combinations (FSM), and real-time feedback. The "Glassmorphism" design ensures maximum readability from a distance during intense physical effort.*

![Workout Interface](./assets/05-workout-interface.png)

### 6. Post-Workout Analysis
*Cool-down and data collection phase. The user evaluates the intensity and their fitness state, feeding the algorithm to adjust future sessions and ensure constant progression without the risk of overtraining.*

![Post-Workout Analysis](./assets/06-post-workout-analysis.png)
